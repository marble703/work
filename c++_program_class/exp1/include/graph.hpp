...
public:
    ...
    // 绕任意旋转中心逆时针旋转angle度
    void rotate(const Point& rotate_center, double angle) {
    ...
    // 以任意点为中心进行镜像
    void mirror(const Point& mirror_center) {
        x = 2 * mirror_center.x - x;
        y = 2 * mirror_center.y - y;
    }
    // 沿图像中心缩放
    void scale(const Point& scale_center, double factor) {
...
public:
    ...
    Line(const Line& other):
        Line(other.start_Point, other.end_Point) {
        Line_count++;
    }
    ~Line() {
        Line_count--;
...
public:
    ...
    Circle(const Circle& other):
        Circle(other.Circle_center, other.radius) {
        Circle_count++;
    }
    ~Circle() {
        Circle_count--;
...
public:
    ...
    Rectangle(const Rectangle& other):
        Rectangle(other.Rect[0], other.Rect[1], other.Rect[2], other.Rect[3]) {
        Rectangle_count++;
    }
    ~Rectangle() {
        Rectangle_count--;
...
private:
    // 对Rect的顶点进行排序，使其按左下角、左上角、右上角、右下角的顺序，并保证 Rect[0], Rect[1] 的 x < Rect[2], Rect[3] 的 x
    void sortRectanglePoints(Point rect[4]) {
        std::sort(rect, rect + 4, [](const Point& p1, const Point& p2) { return p1.x < p2.x; });
    ...
    bool isRectangle() {
        ...
        double side1 = calculateDistance(Rect[0], Rect[1]);
        double side2 = calculateDistance(Rect[1], Rect[2]);
        double side3 = calculateDistance(Rect[2], Rect[3]);
        double side4 = calculateDistance(Rect[3], Rect[0]);
        double diag1 = calculateDistance(Rect[0], Rect[2]);
        double diag2 = calculateDistance(Rect[1], Rect[3]);
...
public:
    ...
    Triangle(const Triangle& other):
        Triangle(other.Tri[0], other.Tri[1], other.Tri[2]) {
        Triangle_count++;
    }
    ~Triangle() {
        Triangle_count--;
...
void drawCircle(const Circle& circle, Color color) {
    ...
    for (int i = 0; i < 100; i++) {
        ...
        glVertex2d(circle.Circle_center.x + circle.radius * cos(angle), circle.Circle_center.y + circle.radius * sin(angle));
    }
    glEnd();
}
